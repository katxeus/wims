...
from wimsintegrate.core.cloud_api import CloudException, CloudNetworkAccessManager
from wimsintegrate.core.cloud_project import CloudProject, ProjectFile, ProjectFileCheckout
from wimsintegrate.core.cloud_transferrer import FileTransfer
from wimsintegrate.gui.cloud_create_project_widget import CloudCreateProjectWidget
...
from wimsintegrate.utils.cloud_utils import LocalDirFeedback, closure, local_dir_feedback
from wimsintegrate.utils.permissions import can_delete_project
from wimsintegrate.utils.qt_utils import rounded_pixmap
...
class CloudProjectsDialog(QDialog, CloudProjectsDialogUi):
    ...
    def __init__(
        ...
        self.buttonBox.button(QDialogButtonBox.Help).clicked.connect(
            lambda: QDesktopServices.openUrl(QUrl("https://docs.wims.memd.go.ug/"))
        )
        ...
        if self.current_cloud_project.cloud_files is not None:
            ...
            if not project_filename:
                iface.messageBar().pushInfo(
                    f'WIMSIntegrate "{self.current_cloud_project.name}":',
                    self.tr(
                        "Cannot find local project file. WIMSIntegrate will first download the project."
                    ),
                )
                return
    ...
    def select_local_dir(self) -> Optional[str]:
        ...
        if self.current_cloud_project is None or (
            ...
            while local_dir is None:
                local_dir = QFileDialog.getExistingDirectory(
                    self, self.tr("Upload local project to WIMS Server…"), initial_path
                )
                ...
                title = self.tr("Cannot upload local WIMSIntegrate directory")
        ...
        # cloud project exists and has files in it, so checkout in an empty dir
            ...
            while local_dir is None:
                local_dir = QFileDialog.getExistingDirectory(
                    self, self.tr("Save WIMS project to…"), initial_path
                )
                ...
                if list(Path(local_dir).iterdir()):
                    ...
                    title = self.tr("WIMSIntegrate checkout prefers an empty directory")
                    answer = None
    ...
    def on_project_delete_button_clicked(self) -> None:
        ...
        def ask(maybe_warning: str = ""):
            delete_msg = self.tr("Delete WIMS project")
            are_you_sure = self.tr(
                "Are you sure you want to delete this WIMS project?"
            )
            confirm_with = self.tr("To confirm deletion, please type")
            reassuring_remark = self.tr(
                "The project will be permanently deleted from WIMS, your local copy will remain unaffected"
            )
