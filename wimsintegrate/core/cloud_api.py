...
from wimsintegrate.utils.qt_utils import strip_html
...
class CloudNetworkAccessManager(QObject):
    ...
    def set_url(self, server_url: str) -> None:
        ...
        # WIMSIntegrate will automatically append `/api/v1` to the path, so prevent double append like `/api/v1/api/v1`.
        if p.path.startswith("/api/v1"):
            self.url = f"{p.scheme or 'http'}://{p.netloc}/"
        else:
            self.url = f"{p.scheme or 'http'}://{p.netloc}{p.path}"
        ...
        if self._projects:
            for project in self._projects:
                ...
                for project_child_dirpath in project_dirpath.glob("**/"):
                    ...
                    # ignore WIMSIntegrate caches
                    if project_child_dirname.startswith(
                        str(project_dirpath.joinpath(".wimsintegrate"))
                    ):
                        continue
