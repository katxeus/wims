...
class CloudTransferrer(QObject):
    ...
    def __init__(
        ...
        self.replies = []
        self.temp_dir = Path(cloud_project.local_dir).joinpath(".wimsintegrate")
    ...
    def import_qfield_project(self) -> bool:
        try:
            ...
            try:
                ...
                self.error_message = self.tr(
                    'Failed to restore the backup. You project might be corrupted! Please check ".wimsintegrate/backup" directory and try to copy the files back manually.'
                )
                self.error.emit(
                    self.error_message,
                    errInner,
